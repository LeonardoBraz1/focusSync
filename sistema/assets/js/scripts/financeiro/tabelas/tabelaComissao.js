$(document).ready(function () {
  ...
  function obterComissao() {
    var startDate1 = $('input[type="date"]').eq(0).val();
    var endDate1 = $('input[type="date"]').eq(1).val();
  
    $.ajax({
      url: "../../controllers/ComissaoController.php",
      type: "POST",
      data: { action: "obterComissoes", startDate1: startDate1, endDate1: endDate1 },
      dataType: "html",
      success: function (response) {
        $("#sampleTable").DataTable().destroy();
        $("#comissaoTableBody").html(response);
      ...
      error: function () {
        alert("Ocorreu um erro ao obter as comissão.");
        ocultarSpinner();
      },
    });
  }
  ...
  function mostrarSpinner() {
    $("#loadingIndicator17").show();
  }
  
  function ocultarSpinner() {
    $("#loadingIndicator17").hide();
  }